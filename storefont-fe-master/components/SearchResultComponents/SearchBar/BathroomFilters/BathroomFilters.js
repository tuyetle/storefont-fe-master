...
class BathroomFilters extends Component {
  constructor(props) {
    super(props);
    let arr = [];
    const {
  ...
  render() {
    return (
      <section className={`${styles.bathroomFilters}`}>
        <div>
          {
            this.props.bathroomOptions.map(({ label, value }) => (
...
BathroomFilters.propTypes = {
  fieldApi: PropTypes.object.isRequired,
  defaultValue: PropTypes.array,
  bathroomOptions: PropTypes.arrayOf(PropTypes.shape({
    value: PropTypes.number.isRequired,
    label: PropTypes.string.isRequired,
  }).isRequired).isRequired,
};
BathroomFilters.defaultProps = {
  defaultValue: [],
};
